...
const CartCard: React.FC<CartCardProps> = (props) => {
  ...
  const isInWishlist = (product: CartItem): boolean => {
  ...
  return (
    <>
      ...
      {data.map((slide, index) => (
        <View key={`${index}-${slide._id}`} style={styles.card}>
      ...
      <View style={styles.totalCard}>
        ...
        <Text style={[styles.cardText, { paddingRight: 5, marginRight: 5 }]}>Total: ${data.map(item => parseFloat(String(item.numericPrice || item.price || 0)) * (item.quantity || 1)).reduce((a, b) => a + b, 0).toFixed(2)}</Text>
      </View>
    </ScrollView>
