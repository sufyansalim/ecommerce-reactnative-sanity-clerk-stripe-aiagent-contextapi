...
import { Product, Brand, Celebrity, TvShow, Category } from '../types';
...
// ============================================
// Mock Celebrity Interface
// ============================================
interface MockCelebrity {
  id: number;
  name: string;
  uri: string;
  banner: string;
...
// ============================================
export const mockCelebrities: MockCelebrity[] = [
  {
    id: 1,
...
// Get product by ID
export const getMockProduct = (id: number): Promise<MockProduct | null> => {
...
export const getMockProducts = (ids: number[]): Promise<MockProduct[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      const products = ids.map(id => mockProducts.find(p => p.id === id)).filter((p): p is MockProduct => p !== undefined);
      resolve(products);
    }, 300);
  });
};
...
export const getMockProductsByCategory = (category: string): Promise<MockProduct[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
...
// Get all products
export const getAllMockProducts = (): Promise<MockProduct[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
...
export const getMockTwoTvProducts = (category?: string): MockProduct[] => {
...
// Get brand by ID
export const getMockBrand = (id: number): MockBrand | undefined => {
  return mockBrands.find(b => b.id === id);
};

// Get celebrity by ID  
export const getMockCelebrity = (id: number): MockCelebrity | undefined => {
