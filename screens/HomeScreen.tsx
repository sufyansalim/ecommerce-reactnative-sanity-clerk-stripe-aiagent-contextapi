...
import { Celebrity, Brand, Product } from '../types';
...
const HomeScreen: React.FC = () => {
  ...
  const slideData: SlideItem[] = brands.slice(0, 6).map((brand) => ({
    uri: brand.image || '',
    text: brand.name || '',
  }));

  // Transform products for grid display
  const productData: TransformedProduct[] = products.slice(0, 4).map((product) => ({
    ...product,
    id: product._id,
    uri: product.image || '',
    name: product.title,
      ...
      <ScrollView
          ...
          <Grid style={styles.celebGrid}>
            {homeCelebrities.slice(0, 3).map((celeb) => (
              <Col key={celeb._id} style={styles.celebImageCol}>
                <TouchableOpacity
                  onPress={() =>
                    navigation.navigate('CelebrityProduct', {
                      celebrity: celeb,
                      banner: celeb.banner,
        ...
        <View style={{ flex: 0.3, paddingTop: 5, marginTop: 5 }}>
          {productData.length > 0 ? (
            <TwoColProducts navigation={navigation} data={productData} home={true} />
          ) : (
            <View style={{ paddingVertical: 20, alignItems: 'center' }}>
