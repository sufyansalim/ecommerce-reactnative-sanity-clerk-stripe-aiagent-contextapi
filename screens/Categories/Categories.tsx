...
const Categories: React.FC<CategoriesProps> = ({ navigation }) => {
  ...
  const loadCategories = async (): Promise<void> => {
    try {
      ...
      const formattedCategories: CategoryItem[] = data.map((cat: any) => ({
        id: cat._id,
        name: cat.name,
        uri: cat.image,
  ...
  const handleImageClick = (categoryName: string): void => {
    navigation.navigate('CategoryProducts', { category: categoryName });
  };

  const renderImages = (data: CategoryItem[]): React.ReactElement[] => {
