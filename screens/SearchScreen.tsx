...
const SearchScreen: React.FC = () => {
  ...
  const handleSearch = async (): Promise<void> => {
    ...
    try {
      const data = await aiSearch(query, userId || undefined);
      setResults(data);
    } catch (err: any) {
  ...
  const handleExampleQuery = (exampleQuery: string): void => {
  ...
  const handleSearchWithQuery = async (searchQuery: string): Promise<void> => {
    ...
    try {
      const data = await aiSearch(searchQuery, userId || undefined);
      setResults(data);
    } catch (err: any) {
      setError(err.message || 'Search failed. Please try again.');
    } finally {
  ...
  const renderExampleQueries = (): React.ReactElement => (
                  ...
                  >
                    ...
                    <View style={styles.productInfo}>
                      ...
                      <Text style={styles.productCategory}>
                        {typeof item.category === 'string' ? item.category : item.category?.name} {item.brand?.name ? `• ${item.brand.name}` : ''}
                      </Text>
