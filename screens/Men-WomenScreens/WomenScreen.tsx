...
// Get products 5-8 for women's home display (Unsplash URLs)
const DATA = mockProducts.slice(4, 8).map((product) => ({
...
// Get celebrities 7-12 for women's home display (different from men's)
const homeCelebrities = mockCelebrities.slice(6, 12);

interface WomenScreenProps {
  navigation?: NavigationProp<any>;
}

const WomenScreen: React.FC<WomenScreenProps> = (props) => {
      ...
      >
        <View style={{flex:0.05, alignItems: 'center', justifyContent: 'center', marginBottom: 10, marginTop: 10}}>
          <Grid style={{alignItems: 'center',}}>
            <Col style={styles.genderTabs}>
              <TouchableOpacity onPress={() => {navigation.navigate('Main', { screen: 'HomeStack' } as any)}}>
                {toggle ? <Text style={[styles.textStyle, {fontWeight: "normal"}]}>Men</Text> : <Text style={styles.textStyle}>Men</Text>}
              </TouchableOpacity>
            </Col>
            <Col style={styles.genderTabs}>
              <TouchableOpacity>
                {toggle ? <Text style={[styles.textStyle, {color: Colors.accent}]}>Women</Text> : <Text style={styles.textStyle}>Women</Text>}
              </TouchableOpacity>
            </Col>
          </Grid>
        </View>
        <View style={{flex:0.3, alignItems: 'center', justifyContent: 'center',}}>
          <Image 
            ...
            source={require('../../assets/images/home/bannerf.jpg')}
          />
        </View>
        ...
        <View style={{flex:0.05, alignItems: 'center', justifyContent: 'center', marginBottom: 5, marginTop: 5}}>
          <Grid>
            ...
            <Col style={[styles.genderTabs, {alignItems:'flex-end', paddingTop:5, paddingRight:10, marginRight:10}]}>
              <TouchableOpacity onPress={() => {navigation.navigate('CelebritiesStack' as never)}}>
          ...
          <Grid style={styles.celebGrid}>
            {homeCelebrities.slice(0, 3).map((celeb, index) => (
              <Col key={celeb.id} style={styles.celebImageCol}>
                <TouchableOpacity onPress={() => navigation.navigate('CelebrityProduct', { 
                  banner: celeb.banner, 
        ...
        <View style={{flex:0.3, paddingTop:5, marginTop:5,}}>
          <TwoColProducts navigation={navigation} data={DATA as any} home={true}/>
        </View>
