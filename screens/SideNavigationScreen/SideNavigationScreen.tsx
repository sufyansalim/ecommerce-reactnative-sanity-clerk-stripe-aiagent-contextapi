...
interface SideNavigationScreenProps {
    navigation?: NavigationProp<any>;
}

const SideNavigationScreen: React.FC<SideNavigationScreenProps> = (props) => {
    ...
    const handleSignOut = async (): Promise<void> => {
        try {
            await signOut();
            navigation.navigate('Main', { screen: 'HomeStack' } as any);
        } catch (err) {
    ...
    const handleSignIn = (): void => {
        navigation.navigate('AuthSignin' as never);
    };

    // Determine the profile image to display
    const displayImage: ImageSourcePropType = profileImage 
        ? { uri: profileImage } 
        : userImage 
            ? { uri: userImage }
        ...
        <CustomHeader navigation={navigation} >
            ...
            <View style={{backgroundColor: Colors.backgroundDark, flex: 0.3, width: "100%"}} >
                ...
                <View style={{flex: 0.7, backgroundColor: Colors.background}}>
                    <TouchableOpacity style={{flexDirection: 'row', marginLeft: 5,marginTop: 5, marginBottom: 5}} onPress ={() => {navigation.navigate('WishlistScreen' as never) }}>
                        <Image style={styles.imageContainer2} source={require('../../assets/menu/My-Wishlist.png')}/>
                        <Text style={styles.text}>My Wishlist</Text>
                    </TouchableOpacity>
                    <Separator/>
                        <TouchableOpacity style={{flexDirection: 'row' ,marginLeft: 5,marginTop: 5, marginBottom: 5}} onPress={() => navigation.navigate('MyOrderScreen' as never)}>
                            <Image style={styles.imageContainer2} source={require('../../assets/menu/My-Order.png')}/>
                            <Text style={styles.text}>My Order</Text>
                        </TouchableOpacity>
                    <Separator/>
                    <TouchableOpacity style={{flexDirection: 'row', marginLeft: 5,marginTop: 5, marginBottom: 5 }} onPress={() => {navigation.navigate('TV' as never)}}>
                        <Image style={styles.imageContainer2} source={require('../../assets/menu/tv.png')}/>
                        <Text style={styles.text}>TV</Text>
                    </TouchableOpacity>
                    <Separator/>
                    <TouchableOpacity style={{flexDirection: 'row', marginLeft: 5,marginTop: 5, marginBottom: 5 }}   onPress={() => navigation.navigate('AboutUsScreen' as never)}>
                    ...
                    <Separator/>
                    <TouchableOpacity style={{flexDirection: 'row', marginLeft: 5,marginTop: 5, marginBottom: 5 }}   onPress={() => navigation.navigate('ShopTheLookScreen' as never)}>
                        <Image style={styles.imageContainer2} source={require('../../assets/menu/tv.png')}/>
