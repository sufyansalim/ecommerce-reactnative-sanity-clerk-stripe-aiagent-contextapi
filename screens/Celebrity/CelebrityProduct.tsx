...
interface CelebrityProductProps {
  navigation: NavigationProp<any>;
  route: RouteProp<any, 'CelebrityProduct'>;
}

const CelebrityProduct: React.FC<CelebrityProductProps> = ({
  navigation,
  route,
}) => {
  ...
  const [celebrityProducts, setCelebrityProducts] = useState<Product[]>(products);
  ...
  const filteredAndSortedProducts = useMemo((): Product[] => {
    if (!celebrityProducts || !Array.isArray(celebrityProducts)) return [];
    ...
    return result;
  }, [celebrityProducts, selectedSort, selectedCategory]);

  const handleSortSelect = (sortId: string): void => {
    setSelectedSort(sortId);
    setSortModalVisible(false);
  };

  const handleFilterSelect = (categoryId: string): void => {
  ...
  return (
    <CustomHeader navigation={navigation}>
      {loading ? (
        <LoadingScreen message="Loading celebrity products..." />
      ) : (
        <ScrollView
          ...
          <View style={{ flex: 0.5 }}>
            {filteredAndSortedProducts && filteredAndSortedProducts.length > 0 ? (
              ...
              <EmptyListView 
                ...
                message="This celebrity has no products yet"
                showActions={false}
              />
            )}
          </View>
...
export default CelebrityProduct;
